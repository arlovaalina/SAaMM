...
def generate_process_time(requests_amount, mean, standard_deviation):
    N = 6
    numbers = generate_numbers(A_VALUE, R_VALUE, M_VALYE, requests_amount * N)
    result = []
...
def start_algorithm(root, mean, standard_deviation, requests_amount):
    channels = [0, 0, 0]
    process_time = [round(x) for x in generate_process_time(requests_amount, mean * 60, standard_deviation * 60)]
    current_minute = 0
    processed_requests = 0
    rejected_requests = 0
    total_busy_channels_amount = 0

    while (processed_requests < requests_amount):
        if (current_minute % 30 == 0):
            free_channel_index = get_free_channel(channels)
            if free_channel_index is None:
                rejected_requests += 1
            else:
                channels[free_channel_index] = process_time[processed_requests]
    ...
    busy_channels_amount = total_busy_channels_amount / (3 * current_minute)
    absolute_throughput = 0
...
def initialize_window():
    ...
    amount_label = Label(root, text = "Количество заявок, N").place(x = 10, y = 110)
